...
!--------------------------------------------------------------------
real :: bav, b2av, bi2a, rbt, bgradp, dpsidr, rnq
real :: fc, gclass, r2i
...
subroutine geom()
  ...
  real :: bmax, tmax, dthet, thet, twopi, bdum
  real :: err1, err2, lam, dlam, fdenom, fco, pi
  real :: dum
  
  real :: b(nmaxgr), theta(nmaxgr),sin1(nmaxgr),sin2(nmaxgr)
  real :: cos1(nmaxgr), cos2(nmaxgr), a(5)

  integer :: ngr,i,j,nc,new_read,ierr
  ...
  case(1) 
    ...
    do while (err2.gt.eps)  
      ...
      err1 = 0.
      do i = 2, ngr-1
        err1 = max(err1, abs( (b(i-1) - 2.*b(i) + b(i+1))/(dthet**2) ))
      end do 
      err2 = max(dthet**2*err1/(abs(bav)), err2)
      ngr = ngr * 2
...
!   !20000 continue
   ...
   call neo_abort('The ASDEX geometry interface (isel=3) is &
...
subroutine visgeom(theta,B)
!--------------------------------------------------------------------
!      SUBROUTINE THAT MUST BE SUPLIED BY THE USER.
!
!      INPUT:  RHO     : THE FLUX SURFACE LABEL.
!              THET    : ANGLE IN HAMADA COORDINATES B GRAD THET = A 
!                        FLUX FUNCTION. THET IS ASSUMED TO LIE IN THE 
!                        RANGE [0,1] (AND CYCLIC)
!      OUTPUT: BGRADP  : THE FLUX FUNCTION B GRAD THET
!              RBT     : THE FLUX FUNCTION R TIMES B_T
!              B         THE MAGNETIC FIELD IN (RHO,THET) THE THIRD 
!                        COORDINATE IS ASSUMED TO BE INGNORABLE
!              DPSIDR  : D PSI / D RHO: A FLUX FUNCTION
!
!
!      BELOW THE QUANTITIES ARE CACULATED FOR CIRCULAR SURFACES WITH
!        BT : THE TOROIDAL MAGNETIC FIELD ON THE MAGNETIC AXIS
!        RN : THE MAJOR RADIUS OF THE MAGNETIC AXIS
!        Q  : THE SAFETY FACTOR OF THE SURFACE
!        EPS: THE INVERSE ASPACT RATIO ( = RHO )
!
!--------------------------------------------------------------------
      use init, only : rho, rn, e, q, bn
      IMPLICIT NONE
      real, intent(out) ::  B
      real :: theta, bt, chi, chio, twoPi
      integer :: itel


      

      BT = bn * SQRT(1. - E**2/(Q**2*(1.-E**2) + E**2))
...
!     THE VALUE OF THE FLUX FUNCTION B GRAD P
      BGRADP = BT / ( TWOPI * Q * RN * SQRT(1-E**2) )  

!     THE VALUE OF DPSIDR
      DPSIDR = RBT *E / (Q * SQRT(1-E**2))

!     CALCULATE THE POLOIDAL ANGLE
      CHIO = 2.*TWOPI
      CHI = TWOPI*THETA
      itel = 0
      do while( abs(chi-chio).gt.1E-6 )
        itel = itel + 1
        IF (ITEL.GT.1000) STOP 'ERR 1: NO CONVERGENCE IN VISGEOM'
        CHIO = CHI
        CHI = TWOPI*THETA - E*SIN(CHI)
       end do
